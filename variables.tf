...
variable "irsa_policy_enabled" {
  type        = bool
  default     = false
  description = "Whether to create opinionated policy to allow AWS operations. Mutually exclusive with irsa_assume_role_enabled"
}

variable "irsa_policy" {
  type        = string
  default     = ""
  description = "Policy to be attached to the default role. Applied only if irsa_policy_enabled is enabled"
}
...
variable "irsa_assume_role_arn" {
  type        = string
  default     = ""
  description = "Assume role ARN. Assume role must be enabled. Applied only if irsa_assume_role_enabled is enabled"
}
